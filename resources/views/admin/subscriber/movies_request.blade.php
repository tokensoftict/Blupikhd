        ...
        <table class="table convert-data-table table-striped">
            ...
            @foreach($movies_requests as $movies_request)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('subscribers.delete_movies_request',$movies_request->id) }}?page=movies" class="btn btn-danger btn-sm">Delete</a>
                            @if($movies_request->status == "PENDING")
                                <a href="{{ route('subscribers.approve_movies_request',$movies_request->id) }}?page=movies" class="btn btn-success btn-sm">Approve Request</a>
                             @endif
                            @if($movies_request->status == "APPROVED")
                                <a href="{{ route('subscribers.schedule_movies_request',$movies_request->id) }}?page=movies" class="btn btn-primary btn-sm">Schedule Request</a>
                            @endif
                        </td>
                    </tr>
                @php
