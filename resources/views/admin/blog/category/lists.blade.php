...
@section('innerTitle',"List Category")
@section('breadcrumb',"")
...
@section('panel_controls')
    <a href="#" id="create" class="btn btn-sm btn-success add" data-toggle="modal" data-target="#id01" data-link="{{ route('category.store') }}">Add New</a>
@endsection
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody class="data-rows">
                @foreach($categories as $category)
                    <tr id="{{ $category->id }}_tr">
                        <td data-name="name">{{ $category->name }}</td>
                        <td data-name="slug">{{ $category->slug }}</td>
                        <td>
                            <a id="edit" data-toggle="modal" data-target="#id01" class="btn btn-primary btn-sm" href="#" data-link="{{ route('category.edit',$category->id) }}">Edit</a>
                            <a id="delete" data-toggle="modal" data-target="#id01" class="btn btn-danger btn-sm" href="#" data-link="{{ route('category.delete',$category->id) }}">Delete</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <script type="text/javascript">
        $(document).on("click", "a", function(){
            ...
            if(modalType==='create' || modalType==='edit' || modalType==='delete'){
                ...
                $('#modal-head').text(modalType.toUpperCase()+ " CATEGORY");
                if(modalType==='create'){
                ...
                success: function(data){
                    if(data['type']==='create'){
                        $('.data-rows').prepend(
                            ...
                            '<td class="data-cell" data-name="slug">'+ data['slug'] +'</td>'+
                            '<td class="data-cell data-cell-action">'+
                            '<a id="edit" data-toggle="modal" data-target="#id01" class="btn btn-sm btn-primary" data-link="{{ asset("category") }}/'+ data['id'] +' /edit">Edit</a> '+
                            '<a id="delete" data-toggle="modal" data-target="#id01" class="btn btn-sm btn-danger" data-link="{{ asset("category") }}/'+ data['id'] +' /delete">Delete</a>'+
                            '</td>'+
                            '</tr>'
                        );
                    ...
                    if(data['type']==='edit'){
                        ...
                        $('#'+data['id']+'_tr').find('td[data-name="slug"]').text(data['slug']);
                        $('.do-close').trigger('click');
                    }
                    if(data['type']==='error'){
